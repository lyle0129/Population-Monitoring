...
static volatile const char   *fifo_tx_buf6 = 0;
static volatile unsigned int  fifo_tx_len6 = 0;
 ...
 */
#define RXFIFO_LEN6	64		// Must be a power of two
#define TXFIFO_LEN6	32		// Must be a power of two
...
void usart6_init(void)
{
	...
	/*
	 * PA11 is used for TX output of the USART6. As a result, we need to do
	 * the following:
	 *
	...
	/*
	 * P12 is used for RX input of the USART. To enable break detection
...
void usart6_exit(void){
	...
	RCC->APB2RSTR &= ~(1 << 5);
	RCC->APB2ENR  &= ~(1 << 5);	// Disable USART6 peripheral

	// Revert the GPIO pins to input.
	GPIOA->MODER &= ~(0b11 << 22);	// Set PA2 as input...
	GPIOA->MODER &= ~(0b11 << 24);	// Set PA3 as input...
	GPIOA->OTYPER  &= ~(1 << 11);
	GPIOA->OSPEEDR &= ~(0b11 << 22);
	GPIOA->PUPDR &= ~(0b11 << 24);
	// Revert USART6 alternate function
	GPIOA->AFR[1] &= ~(0x000FF000);   // Clear AF bits for PA11 (USART6_TX)
	/*
